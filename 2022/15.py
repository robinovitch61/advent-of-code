...
def get_union(sensors, n):
    ...
    for s, b in sensors.items():
        mid_x = s[0]
        if man((s[0], n), s) <= man(s, b):
            dx = man(s, b) - abs(s[1] - n)
            spans[(s, b)] = (mid_x - dx, mid_x + dx)
        else:
            spans[(s, b)] = None

    union = []
    for begin, end in sorted([s for s in spans.values() if s]):
...
# Advent of Code[About][Events][Shop][Settings][Log Out]robinovitch61 (AoC++) 29*
