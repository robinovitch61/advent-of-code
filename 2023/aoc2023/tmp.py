...
text = """...317..........214.....................................751.................................630...479..205....41.993............416.........
...
def getnum(line, i):
    if i < 0 or i > len(line) or not line[i].isdigit():
    ...
    return int(line[start + 1: end]), start + 1, end

res = 0
nums = set()
for y, line in enumerate(lines):
    for x, c in enumerate(line):
        n, start, end = getnum(line, x)
        if n:
            nums.add((n, start, end, y))

res = 0
for n, start, end, y in nums:
    found = False
    for x in range(start, end):
        for dx, dy in (
            ...
            if (y+dy, x+dx) in symbols and not found:
                res += n
                found = True
...
for y, line in enumerate(lines):
    for x, c in enumerate(line):
        if c == "*":
            ...
            for dx in (-1, 0, 1):
                if y > 0:
                    n, start, end = getnum(lines[y-1], x + dx)
                    if n:
                        starToNums[(y, x)].add((n, start, end))
                if y < len(lines) - 1:
                    n, start, end = getnum(lines[y+1], x + dx)
                    if n:
                        starToNums[(y, x)].add((n, start, end))
                n, start, end = getnum(line, x + dx)
                if n:
                    starToNums[(y, x)].add((n, start, end))
print(starToNums)
res = 0
for _, v in starToNums.items():
    if len(v) == 2:
        v = list(v)
        res += v[0][0] * v[1][0]
print(res)
